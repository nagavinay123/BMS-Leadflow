...
class AutoOutreachRequest(BaseModel):
    run_id:     Optional[str]  = None
    dry_run:    bool           = True   # ALWAYS default True — safety first
    user_email: Optional[str] = None


@app.post("/api/auto-outreach")
def auto_outreach(request: AutoOutreachRequest):
    """
    Identify eligible leads from a search run (or recent companies) and
    optionally send personalised emails to each one.

    Eligibility gates (all must pass):
      1. score >= OUTREACH_SCORE_THRESHOLD
      2. contact_email present (verified email discovered)
      3. ch_matched == True  (PECR incorporated-entity gate)
      4. outreach_status is None / "none"  (not already contacted)
      5. Not on suppression list (by email domain or company number)

    dry_run=True (default): generates + saves drafts but does NOT send
    or change outreach_status.  Safe to run any time.

    dry_run=False: generates drafts, marks status → "emailed", records
    an event in the audit trail.  Idempotent — companies already emailed
    are skipped.

    Returns:
      { dry_run, found, eligible, attempted, sent, skipped, failed, details }
    """
    from database import supabase as db, _merge_audit
    from outreach import OUTREACH_SCORE_THRESHOLD, save_email_draft, update_outreach_status
    from email_templates import generate_email
    from datetime import datetime, timezone

    dry_run = request.dry_run  # honour request; default is True

    # ── 1. Fetch candidate companies ──────────────────────────────
    try:
        query = (
            db.table("companies")
            .select("*, website_audits(*)")
            .gte("score", OUTREACH_SCORE_THRESHOLD)
        )
        if request.run_id:
            query = query.eq("run_id", request.run_id)

        rows = query.order("score", desc=True).limit(500).execute().data or []
        companies = [_merge_audit(r) for r in rows]
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to fetch companies: {e}")

    found = len(companies)

    # ── 2. Build suppression set ──────────────────────────────────
    try:
        suppression = get_suppression_list(limit=1000)
        suppressed_emails   = {s["email"].lower()   for s in suppression if s.get("email")}
        suppressed_domains  = {s["domain"].lower()  for s in suppression if s.get("domain")}
        suppressed_ch_nums  = {s["company_number"]  for s in suppression if s.get("company_number")}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to load suppression list: {e}")

    # ── 3. Eligibility filter ─────────────────────────────────────
    eligible = []
    for c in companies:
        email      = (c.get("contact_email") or "").strip().lower()
        ch_matched = c.get("ch_matched", False)
        status     = (c.get("outreach_status") or "none").lower()
        ch_number  = c.get("company_number") or ""

        if not email:
            continue  # no verified email
        if not ch_matched:
            continue  # PECR gate: must be an incorporated entity
        ...
        if email_domain and email_domain in suppressed_domains:
            continue
        if ch_number and ch_number in suppressed_ch_nums:
            continue
    ...
    # ── 4. Process each eligible company ─────────────────────────
    sent    = 0
    skipped = 0
    failed  = 0
    details = []
    ...
    for company in eligible:
        ...
        try:
            # Generate personalised email draft
            draft = generate_email(company)
            save_email_draft(
                company_id,
                draft["subject"],
                draft["body"],
    ...
    try:
        ...
        pass  # don't fail the whole request over logging
